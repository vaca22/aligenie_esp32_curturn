...
void user_genie_event_handle(genie_event_t event, void *p_arg)
{
    ...
    ESP_LOGE("fuck","fuck");
    ESP_LOGE(TAG, "event: %02d, %p", event, p_arg);
    if(event==GENIE_EVT_SDK_VENDOR_MSG){
        ESP_LOGE("fuck","fuckyout");
    }
    genie_model_msg_t *p_msg;
    switch (event) {


        case GENIE_EVT_SDK_VENDOR_MSG:
            p_msg=(genie_model_msg_t *)p_arg;
            uint8_t * p_data = p_msg->data;
            ESP_LOGI(TAG, "opcode:0x%x, tid:%d, len:%d", p_msg->opid, p_msg->tid, p_msg->len);
            if (p_data && p_msg->len) {
                ESP_LOGE(TAG, "payload: %s", util_hex2str(p_data, p_msg->len));
            }
            ESP_LOGE(TAG, "opco   fuck   de:%d",p_data[2]);
            gpio_set_level(15, 1-p_data[2]);
            gpio_set_level(16, 1-p_data[2]);
            gpio_set_level(32, 1-p_data[2]);
            break;
    ...
    case GENIE_EVT_SDK_ACTION_DONE: {
        ESP_LOGE(TAG, "GENIE_EVT_SDK_ACTION_DONE");
    ...
    default:
        ESP_LOGE(TAG, "unhandle this event: %2d", event);
        break;
    }
